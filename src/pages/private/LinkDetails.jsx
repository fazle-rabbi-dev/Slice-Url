...
import { useGetLinkByShortId, useChangeLinkAlias, useDeleteLink } from "@/lib/react-query";
import { Loader, Button, LinkClickDetails } from "@/components";
import { showToast, playSound, generateShortUrl, copyToClipboard } from "@/lib/utils";
...
export const LinkDetails = () => {
  ...
  const navigate = useNavigate();
	...
	const { data: ApiResponse, isPending: isLoadingLink } = useGetLinkByShortId(linkId);
	const { mutateAsync: updateLinkAlias, isPending: isUpdatingLinkAlias } = useChangeLinkAlias();
  ...
  if(!isLoadingLink && !link) {
    return (
        <div className="">
          ...
          <Link className="text-orange-600 font-cascadia hover:underline" to="/">🏕️ Go to home </Link>
        </div>
      )
  }
				...
				<div className="space-y-4">
					...
					<div className="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4 rounded-md shadow-lg dark:bg-orange-900/20 dark:text-orange-400">
						<details>
  						...
  						<h2 className="text-lg font-semibold mb-2">🔥 Special Feature: Track URL Click Sources</h2>
  						<p className="mb-4">
  							Easily track where your short URLs are getting clicks by adding a{" "}
  							<strong>source</strong> query parameter to the end of your URL. This feature helps you
  							understand the traffic source of each click.
  						</p>
  						...
  						<p>
  							...
  							clicks, including the sources of those clicks.
  						</p>
						</details>
					...
					<div>
						...
						<p className="body-text">{new Date(link.createdAt).toLocaleString()}</p>
					</div>
					<div>
						<span className="font-semibold">Last Clicked At:</span>
						<p className="body-text">
							{link.clickedAt.length > 0
								? new Date(link.clickedAt[link.clickedAt.length - 1].time).toLocaleString()
								: "N/A"}
						</p>
					</div>
					<div>
